...
export type UpdateShiftInput = { id: string } & Database['public']['Tables']['shift_assignment']['Update']
  ...
  return useMutation<ShiftAssignment, Error, CreateShiftInput>({
    ...
    onError: (_err, input, context) => {
      const ctx = context as
        | { snapshot: ShiftAssignment[] | undefined; cacheKey: readonly string[] }
        | undefined
...
export function useUpdateShift() {
  ...
  return useMutation<void, Error, UpdateShiftInput>({
    ...
    onMutate: async ({ id, ...fields }) => {
      ...
      queryClient.getQueriesData<ShiftAssignment[]>({ queryKey: queryKeys.shifts.all }).forEach(
        ([key, data]) => {
          if (data) {
