...
const USER_1 = makeUser({
  id: "user-1",
  first_name: "Mario",
  last_name: "Rossi",
});
const USER_2 = makeUser({
  id: "user-2",
  first_name: "Anna",
  last_name: "Bianchi",
});
const USER_3 = makeUser({
  id: "user-3",
  first_name: "Carlo",
  last_name: "Zeri",
});
...
describe("buildShiftGrid", () => {
  ...
  describe("multiple users", () => {
    ...
    it("does not mix assignments between users", () => {
      ...
      const result = buildShiftGrid([a1, a2], [USER_1, USER_2]);
      const rossiRow = result.find(
        (r: ShiftGridRow) => r.user.id === USER_1.id,
      )!;
      const bianchiRow = result.find(
        (r: ShiftGridRow) => r.user.id === USER_2.id,
      )!;
      expect(rossiRow.cells["2026-04-07"].blocks[0].label).toBe("Rossi shift");
      expect(bianchiRow.cells["2026-04-07"].blocks[0].label).toBe(
        "Bianchi shift",
      );
    });
  });
    ...
    it("handles 100 users with 5 assignments each without error", () => {
      const users: User[] = Array.from({ length: 100 }, (_, i) =>
        makeUser({
          id: `user-${i}`,
          first_name: "Name",
          last_name: `Last${String(i).padStart(3, "0")}`,
        }),
      );
      const assignments: ShiftAssignment[] = users.flatMap((u, i) =>
        Array.from({ length: 5 }, (_, j) =>
          makeAssignment({
            id: `a-${i}-${j}`,
            user_id: u.id,
            date: `2026-04-${String(j + 1).padStart(2, "0")}`,
          }),
        ),
      );
