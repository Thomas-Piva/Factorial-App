...
describe('useRealtimeNotifications', () => {
  ...
  it('subscribes to the notification channel for the given user', () => {
    renderHook(() => useRealtimeNotifications('user-1'), { wrapper: makeWrapper() })
  ...
  it('does not subscribe when userId is undefined', () => {
    renderHook(() => useRealtimeNotifications(undefined), { wrapper: makeWrapper() })
    expect(mockChannel).not.toHaveBeenCalled()
  })

  it('unsubscribes on unmount', () => {
    const { unmount } = renderHook(
      () => useRealtimeNotifications('user-1'),
      { wrapper: makeWrapper() }
    )
    unmount()
