...
describe("ColleghiOggiCard", () => {
  ...
  it("shows EmptyState when colleagues list is empty", () => {
    render(
      <ColleghiOggiCard storeId="store-1" colleagues={[]} isLoading={false} />,
      { wrapper },
    );
    expect(
      screen.getByText(/nessun collega in servizio oggi/i),
    ).toBeInTheDocument();
  });

  it("shows a loading spinner while loading", () => {
    render(
      <ColleghiOggiCard storeId="store-1" colleagues={[]} isLoading={true} />,
      { wrapper },
    );
    expect(screen.getByRole("status")).toBeInTheDocument();
  });
...
describe("ComunicazioniCard", () => {
  it("renders a comunicazioni card with placeholder text", () => {
    ...
    expect(
      screen.getByText(/nessuna nuova comunicazione/i),
    ).toBeInTheDocument();
  });
