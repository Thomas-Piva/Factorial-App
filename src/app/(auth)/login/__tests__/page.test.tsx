...
describe('LoginPage', () => {
  ...
  it('shows a validation error when form is submitted empty', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/email obbligatoria|inserisci.*email|email.*richiesta/i)
      ).toBeInTheDocument()
    })
  })
  ...
  it('calls signInWithPassword with correct email and password on submit', async () => {
    ...
    await user.type(screen.getByRole('textbox', { name: /email/i }), 'mario@example.com')
    await user.type(screen.getByLabelText(/password/i), 'secret123')
    await user.click(screen.getByRole('button', { name: /accedi|login|entra/i }))
  ...
  it('shows an error message when authentication fails', async () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(/credenziali non valide/i)
  })
